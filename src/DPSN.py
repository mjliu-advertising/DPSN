...
# ################### Arguments ####################
def parse_args():
    parser = argparse.ArgumentParser(description="Run DPSN.")
...
class NeuralFM(BaseEstimator, TransformerMixin):
    ...
    def _initialize_weights(self):
        ...
        if self.pretrain_flag > 0:  # with pretrain
            ...
            pretrain_file = '../output/pretrain/%s_%d' % (
    ...
    def get_order_block_from_data(self, data, start_index=0, batch_size=-1):  # generate a random block of training data
        # start_index = np.random.randint(0, len(data['Y']) - batch_size)
        X, Y = [], []
        # forward get sample
        i = start_index
...
if __name__ == '__main__':
    ...
    if args.verbose > 0:
        print(
            "DPSN: dataset=%s, hidden_factor=%d, dropout_keep=%s, deep_layers=%s, deep_layers=%s, stack_layers=%s, loss_type=%s, pretrain=%d, #epoch=%d, batch=%d, lr=%.4f, lambda=%.4f, optimizer=%s, batch_norm=%d, activation=%s, early_stop=%d"
